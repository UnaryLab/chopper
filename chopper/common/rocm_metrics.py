...
def derive_lds_stalled_perc(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        (df["SQ_LDS_BANK_CONFLICT"] * 3.125) /
        (df["SQ_LDS_IDX_ACTIVE"] - df["SQ_LDS_BANK_CONFLICT"])
    )
    df["LDS Stalled"] = value

    # stall_cycles = (
    #     df["SQ_LDS_UNALIGNED_STALL"] + df["SQ_LDS_ADDR_CONFLICT"] +
    #     df["SQ_LDS_BANK_CONFLICT"]
    # )
    # total_cycles = stall_cycles + df["SQ_LDS_IDX_ACTIVE"]
    # df["LDS Stalled"] = 100 * (
    #     stall_cycles /
    #     total_cycles
    # )
...
def derive_lds_bank_conflict_perc(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # value = (df["SQ_LDS_BANK_CONFLICT"] * 3.125) / \
    #     (df["SQ_LDS_IDX_ACTIVE"] - df["SQ_LDS_BANK_CONFLICT"])
...
def derive_lds_bw_pop(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "SQ_LDS_IDX_ACTIVE" in df.columns and "SQ_LDS_BANK_CONFLICT" in df.columns and "dur" in df.columns:
        value = 100 * ((
            (df["SQ_LDS_IDX_ACTIVE"] - df["SQ_LDS_BANK_CONFLICT"]
             ) * 4) * lds_banks_per_cu) / (df["dur"].astype(float)*1e3)
...
def derive_lds_bw(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "SQ_LDS_IDX_ACTIVE" in df.columns and "SQ_LDS_BANK_CONFLICT" in df.columns and "dur" in df.columns:
        value = ((
            (df["SQ_LDS_IDX_ACTIVE"] - df["SQ_LDS_BANK_CONFLICT"]
             ) * 4) * lds_banks_per_cu) / (df["dur"].astype(float)*1e-6)
...
def derive_active_cu(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sequencer_cycles = np.floor(
        (4 * df["SQ_BUSY_CU_CYCLES"]) /
        (df["GRBM_GUI_ACTIVE"]/n_xcd)
    )
...
def derive_reported_duration(df: pd.DataFrame) -> pd.DataFrame:
    ...
    n_xcd = 8
    freq = 2100
    df['Reported Duration'] = df["GRBM_GUI_ACTIVE"] / n_xcd / freq
...
def derive_reported_duration_diff(df: pd.DataFrame) -> pd.DataFrame:
    ...
    n_xcd = 8
    freq = 2100
    reported_duration = df["GRBM_GUI_ACTIVE"] / n_xcd / freq
    actual_duration = df["dur"].astype(float)
...
#         diff = (df["SQ_LDS_IDX_ACTIVE"] - df["SQ_LDS_BANK_CONFLICT"])
#         value = df["SQ_LDS_BANK_CONFLICT"] / diff
#         peak = 32
...
def derive_lds_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["LDS Util"] = (
        100 * df["SQ_LDS_IDX_ACTIVE"] /
        (df["GRBM_GUI_ACTIVE"] / n_xcd * cu_per_gpu))
...
def derive_lds_latency(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = df["SQ_INST_LEVEL_LDS"] / df["SQ_INSTS_LDS"]
...
def derive_vmem_latency(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = df["SQ_INST_LEVEL_VMEM"] / df["SQ_INSTS_VMEM"]
...
def derive_l1d_addr_hitrate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (100*df["TCP_UTCL1_TRANSLATION_HIT_sum"] /
             df["TCP_UTCL1_REQUEST_sum"])
    df["L1D Address Translation Hit Rate"] = value


    return df
def derive_vl1d_hit_rate(df: pd.DataFrame) -> pd.DataFrame:
    # test1 = df["TCP_TCC_READ_REQ_sum"]
    # test2 = df["TCP_TCC_WRITE_REQ_sum"]
    # test3 = df["TCP_TCC_ATOMIC_WITH_RET_REQ_sum"]
    # test4 = df["TCP_TCC_ATOMIC_WITHOUT_RET_REQ_sum"]
    # test5 = df["TCP_TOTAL_CACHE_ACCESSES_sum"]
    ...
    value = ((100 - ((100 * (((df["TCP_TCC_READ_REQ_sum"] + df["TCP_TCC_WRITE_REQ_sum"])
              + df["TCP_TCC_ATOMIC_WITH_RET_REQ_sum"]) + df["TCP_TCC_ATOMIC_WITHOUT_RET_REQ_sum"]))
...
def derive_vl1d_bytes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = df["TCP_TOTAL_CACHE_ACCESSES_sum"].astype(float) * 128
    df["vL1D Bytes"] = value
...
def derive_vl1d_bandwidth(df: pd.DataFrame) -> pd.DataFrame:
    assert "TCP_TOTAL_CACHE_ACCESSES_sum" in df.columns and "dur" in df.columns
    value = (df["TCP_TOTAL_CACHE_ACCESSES_sum"].astype(float) *
             128 / (df["dur"].astype(float) * 1e-6))
...
def derive_vl1d_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = df["TCP_GATE_EN2_sum"] * 100 / df["TCP_GATE_EN1_sum"]
    df["vL1D Util"] = value
...
def derive_l2_data_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (100 * df["TCP_PENDING_STALL_CYCLES_sum"] /
             df["TCP_GATE_EN1_sum"])
...
def derive_l2_req_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (100 * df["TCP_TCR_TCP_STALL_CYCLES_sum"] /
             df["TCP_GATE_EN1_sum"])
...
def derive_l2_tag_ram_read_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (100 * df["TCP_READ_TAGCONFLICT_STALL_CYCLES_sum"] /
             df["TCP_GATE_EN1_sum"])
...
def derive_l2_tag_ram_write_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (100 * df["TCP_WRITE_TAGCONFLICT_STALL_CYCLES_sum"] /
             df["TCP_GATE_EN1_sum"])
...
def derive_l2_tag_ram_atomic_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (100 * df["TCP_ATOMIC_TAGCONFLICT_STALL_CYCLES_sum"] /
             df["TCP_GATE_EN1_sum"])
...
def derive_ta_address_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (100 * df["TA_ADDR_STALLED_BY_TC_CYCLES_sum"] /
             (df["GRBM_GUI_ACTIVE"] / n_xcd * cu_per_gpu))
...
def derive_ta_data_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (100 * df["TA_DATA_STALLED_BY_TC_CYCLES_sum"] /
             (df["GRBM_GUI_ACTIVE"] / n_xcd * cu_per_gpu))
...
def derive_ta_dp_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (100 * df["TA_ADDR_STALLED_BY_TD_CYCLES_sum"] /
             (df["GRBM_GUI_ACTIVE"] / n_xcd * cu_per_gpu))
...
def derive_l2_cache_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (100 * df["TCC_BUSY_sum"] /
             (df["GRBM_GUI_ACTIVE"] / n_xcd * total_l2_chan))
    df["L2 Util"] = value
...
def derive_l2_fabric_read_bw(df: pd.DataFrame) -> pd.DataFrame:
    ...
    read_bytes = (128 * df["TCC_BUBBLE_sum"] +
                  64 * (df["TCC_EA0_RDREQ_sum"] - df["TCC_BUBBLE_sum"] - df["TCC_EA0_RDREQ_32B_sum"]) +
                  32 * df["TCC_EA0_RDREQ_32B_sum"])
    value = read_bytes / (df["dur"].astype(float) * 1e3)
...
def derive_l2_fabric_write_bw(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = write_bytes / (df["dur"].astype(float)*1e3)
...
def derive_sl1d_cache_hit_rate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    hits = df["SQC_DCACHE_HITS"]
    misses = (df["SQC_DCACHE_MISSES"] +
              df["SQC_DCACHE_MISSES_DUPLICATE"])
    value = 100 * hits / (hits + misses)
...
def derive_sl1d_cache_bw_pop(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (df["SQC_DCACHE_REQ"] * 100000 / (max_sclk *
             sqc_per_gpu * (df["dur"].astype(float) * 1e3)))
...
def derive_sl1d_l2_bw_pop(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = ((
        df["SQC_TC_DATA_READ_REQ"] +
        df["SQC_TC_DATA_WRITE_REQ"] +
        df["SQC_TC_DATA_ATOMIC_REQ"]) * 100000 /
        (2 * max_sclk * sqc_per_gpu * (df["SQC_DCACHE_REQ"].astype(float) * 1e3)))
...
def derive_l1i_cache_hit_rate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    hits = df["SQC_ICACHE_HITS"]
    misses = (df["SQC_ICACHE_MISSES"] +
              df["SQC_ICACHE_MISSES_DUPLICATE"])
    value = 100 * hits / (hits + misses)
...
def derive_l1i_l2_bw_pop(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        (df["SQC_TC_INST_REQ"] * 100000) /
        (2 * max_sclk * sqc_per_gpu * df["dur"].astype(float)*1e3)
    )
...
def derive_inffab_32b_req(df: pd.DataFrame) -> pd.DataFrame:
    ...
    read_req = df["TCC_EA0_RDREQ_32B_sum"]
    write_req = df["TCC_EA0_WRREQ_sum"] - df["TCC_EA0_WRREQ_64B_sum"]
...
def derive_inffab_64b_req(df: pd.DataFrame) -> pd.DataFrame:
    assert "TCC_BUBBLE_sum" in df.columns and "TCC_EA0_RDREQ_32B_sum" in df.columns and "TCC_EA0_RDREQ_sum" in df.columns and "TCC_EA0_WRREQ_64B_sum" in df.columns and "TCC_EA0_WRREQ_sum" in df.columns
    read_req = (df["TCC_EA0_RDREQ_sum"] -
                df["TCC_BUBBLE_sum"] - df["TCC_EA0_RDREQ_32B_sum"])
    write_req = df["TCC_EA0_WRREQ_64B_sum"]
...
def derive_l2_streaming_req(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["L2 Streaming Req"] = df["TCC_STREAMING_REQ_sum"]
...
def derive_inffab_bw(df: pd.DataFrame) -> pd.DataFrame:
    ...
    b = read_bytes + write_bytes
    df["InfFab Bandwidth"] = b / (df["dur"].astype(float)*1e-6)
...
def derive_ea_uncached_bytes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    b = (df["TCC_EA0_WR_UNCACHED_32B_sum"] +
         df["TCC_EA0_RD_UNCACHED_32B_sum"]) * 32
...
def derive_salu_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = ((100 * df["SQ_ACTIVE_INST_SCA"]) /
             (df["GRBM_GUI_ACTIVE"]/n_xcd * cu_per_gpu))
    df["SALU Util"] = value
...
def derive_valu_cycles(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["VALU Cycles"] = df["SQ_ACTIVE_INST_VALU"]/cu_per_gpu
...
def derive_valu_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = ((100 * df["SQ_ACTIVE_INST_VALU"]) /
             (df["GRBM_GUI_ACTIVE"]/n_xcd * cu_per_gpu))
...
def derive_vmem_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = ((100 * (df["SQ_ACTIVE_INST_FLAT"] + df["SQ_ACTIVE_INST_VMEM"])) /
             (df["GRBM_GUI_ACTIVE"]/n_xcd * cu_per_gpu))
...
def derive_branch_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    n_xcd = 8
    cu_per_gpu = 304
    value = ((100 * df["SQ_ACTIVE_INST_MISC"]) /
             (df["GRBM_GUI_ACTIVE"]/n_xcd * cu_per_gpu))
...
def derive_vmem_issued(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = df["SQ_INSTS_VMEM"] + df["SQ_INSTS_LDS"]
...
def derive_mfma_i8_issued(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = df["SQ_INSTS_VALU_MFMA_I8"]
...
def derive_mfma_f16_issued(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = df["SQ_INSTS_VALU_MFMA_F16"]
...
def derive_mfma_f32_issued(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = df["SQ_INSTS_VALU_MFMA_F32"]
...
def derive_mfma_f64_issued(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = df["SQ_INSTS_VALU_MFMA_F64"]
...
def derive_workgroup_manager_ns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    se_per_gpu = 32
    value = 100*(
        df["SPI_RA_REQ_NO_ALLOC_CSN"] /
        (df["GRBM_SPI_BUSY"] / n_xcd * se_per_gpu)
    )
...
def derive_simd_waveslot_insf(df: pd.DataFrame) -> pd.DataFrame:
    ...
    se_per_gpu = 32
    value = 100*(
        df["SPI_RA_WAVE_SIMD_FULL_CSN"] /
        (df["GRBM_GUI_ACTIVE"] / n_xcd * se_per_gpu)
    )
...
def derive_vgpr_writes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["VGPR Writes"] = 4 * df["SPI_VWC_CSC_WR"] / df["SPI_CSN_WAVE"]
...
def derive_mfma_flop_inst(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cu_per_gpu = 304
    mfma_cycles = (
        df["SQ_INSTS_VALU_MFMA_MOPS_BF16"] * 512 /
...
def derive_vl1_uncached(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        df["TCP_TCC_UC_READ_REQ_sum"] +
        df["TCP_TCC_UC_WRITE_REQ_sum"] +
        df["TCP_TCC_UC_ATOMIC_REQ_sum"]
    )
...
def derive_cpf_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        100 * df["CPF_CPF_STAT_BUSY"] /
        (df["CPF_CPF_STAT_IDLE"] + df["CPF_CPF_STAT_BUSY"])
    )
...
def derive_cpf_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = 100 * df["CPF_CPF_STAT_STALL"] / (df["CPF_CPF_STAT_BUSY"])
...
def derive_cpf_l2_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        100 * df["CPF_CPF_TCIU_BUSY"] /
        (df["CPF_CPF_TCIU_BUSY"] + df["CPF_CPF_TCIU_IDLE"])
    )
...
def derive_cpf_l2_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        100 * df["CPF_CPF_TCIU_STALL"] / (df["CPF_CPF_TCIU_BUSY"])
    )
...
def derive_cpf_utcl1_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        100 * df["CPF_CMP_UTCL1_STALL_ON_TRANSLATION"] /
        (df["CPF_CPF_STAT_BUSY"])
    )
...
def derive_cpc_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        100 * df["CPC_CPC_STAT_BUSY"] /
        (df["CPC_CPC_STAT_IDLE"] + df["CPC_CPC_STAT_BUSY"])
    )
...
def derive_cpc_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        100 * df["CPC_CPC_STAT_STALL"] / df["CPC_CPC_STAT_BUSY"]
    )
...
def derive_cpc_decoding_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        100 * df["CPC_ME1_BUSY_FOR_PACKET_DECODE"] /
        df["CPC_CPC_STAT_BUSY"]
    )
...
def derive_cpc_l2_util(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        100 * df["CPC_CPC_TCIU_BUSY"] /
        (df["CPC_CPC_TCIU_BUSY"] + df["CPC_CPC_TCIU_IDLE"])
    )
...
def derive_cpc_utcl1_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        100 * df["CPC_UTCL1_STALL_ON_TRANSLATION"] /
        df["CPC_CPC_STAT_BUSY"]
    )
...
def derive_cpc_utcl2_stall(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value = (
        100 * df["CPC_CPC_UTCL2IU_BUSY"] /
        (df["CPC_CPC_UTCL2IU_BUSY"] + df["CPC_CPC_UTCL2IU_IDLE"])
    )
...
def derive_vl1d_coalesce(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["vL1D Coalesce"] = ((df["TA_TOTAL_WAVEFRONTS_sum"] * 64)
                           * 100) / (df["TCP_TOTAL_ACCESSES_sum"] * 4)
